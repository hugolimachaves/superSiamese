...
def getOpts(opts):
	...
	opts['instanceSize'] = 295#255
	opts['scoreSize'] = 22#17
	opts['totalStride'] = 8
...
def conservative_similarity(list_zFeat):
	similarity = np.zeros(list_zFeat[0].shape)

	for zFeat in list_zFeat[:(len(list_zFeat)/2)]:
		similarity += zFeat
...
def main(_):
	...
	imgs, targetPosition, targetSize = loadVideoInfo(caminho_dataset, nome_do_video)#loadVideoInfo(opts['seq_base_path'], opts['video'])
	nImgs = len(imgs)
	...
	for frame in range(POSICAO_PRIMEIRO_FRAME, nImgs):
		...
		if frame > POSICAO_PRIMEIRO_FRAME:
			...
			print('tipo xCrops: ',type(xCrops))
			print('shape xCrops: ',xCrops.shape)	
